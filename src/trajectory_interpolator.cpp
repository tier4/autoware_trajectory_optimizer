...
: Node("trajectory_interpolator", options)
{
  ...
  traj_sub_ = create_subscription<Trajectory>(
    "mtr/trajectory", 10, std::bind(&TrajectoryInterpolator::on_traj, this, std::placeholders::_1));
  // interface publisher
  traj_pub_ = create_publisher<Trajectory>("smoothed/mtr/trajectory", 5);
}

Trajectory TrajectoryInterpolator::interpolate_trajectory(
  const Trajectory & input_trajectory, const Odometry & current_odometry)
{
  auto traj_points = autoware::motion_utils::convertToTrajectoryPointArray(input_trajectory);
  ...
  output_traj.header = input_trajectory.header;
  return output_traj;
}

void TrajectoryInterpolator::on_traj(const Trajectory::ConstSharedPtr msg)
{
  ...
  const auto output_traj = interpolate_trajectory(*msg, *current_odometry_ptr_);
